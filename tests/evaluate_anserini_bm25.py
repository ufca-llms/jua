...
index_name = "beir/ulysses"  # beir/scifact
...
chunk_size = 100
results = json.load(open("results/anserini_bm25_ulysses.json", "r")) if os.path.exists("results/anserini_bm25_ulysses.json") else {}
print(len(results), "queries already retrieved, resuming from there...")
for i in tqdm(range(len(results), len(queries), chunk_size)):
    ...
    json.dump(results, open("results/anserini_bm25_ulysses.json", "w"))
